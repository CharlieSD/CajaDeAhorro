angular
    .module('app')
    .service('traker',trakerService);
        

trakerService.$inject = ['$http','$location','$cookies'];
	

function trakerService($http,$location,$cookies){
    var ResponseCookie = {
        find:false,
        response:false
    };
    ...
    return service;			
        ...
        function check(thisOne){
            var checker = $cookies.getObject(thisOne);
        ...
        function create(thisOne,data){
            if(check(thisOne))
                update(thisOne,data);
            else
                $cookies.putObject(thisOne,data);
        }

        function recover(thisOne){

           if(check(thisOne))
                return $cookies.getObject(thisOne);
            else 
                return false;
        }

        function update(thisOne,data){
            if(check(thisOne)){
                del(thisOne);
            }
            $cookies.putObject(thisOne,data);           
        ...
        function del(thisOne){
            $cookies.remove(thisOne);
        }
